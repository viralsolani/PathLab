...
@section('content')
    ...
    @can('role_create')
    <p>
        <a href="{{ route('roles.create') }}" class="btn btn-success">@lang('admin.add_new')</a>
    </p>
    @endcan
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped {{ count($roles) > 0 ? 'datatable' : '' }} dt-select">
                ...
                <tbody>
                    @if (count($roles) > 0)
                        @foreach ($roles as $role)
                            <tr data-entry-id="{{ $role->id }}">
                                ...
                                <td>
                                    @can('role_view')
                                    <a href="{{ route('roles.show',[$role->id]) }}" class="btn btn-xs btn-primary">@lang('admin.view')</a>
                                    @endcan
                                    @can('role_edit')
                                    <a href="{{ route('roles.edit',[$role->id]) }}" class="btn btn-xs btn-info">@lang('admin.edit')</a>
                                    @endcan
                                    @can('role_delete')
                                    {!! Form::open(array(
                    ...
                    @else
                        <tr>
                            <td colspan="3">@lang('admin.no_entries_in_table')</td>
                        </tr>
                    @endif
                </tbody>
            </table>
